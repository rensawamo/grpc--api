...
const getAccountaaa = `-- name: GetAccountaaa :one
SELECT id, owner, balance, currency, created_at FROM accounts
WHERE id = $1 LIMIT 1
`

func (q *Queries) GetAccountaaa(ctx context.Context, id int64) (Account, error) {
	row := q.db.QueryRow(ctx, getAccountaaa, id)
	var i Account
	err := row.Scan(
		&i.ID,
		&i.Owner,
